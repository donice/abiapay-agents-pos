...
var ViewReceiptComponent = function (_a) {
    var id = _a.id;
    var _b = useQuery({
        queryKey: ["receipt", id],
        queryFn: function () {
    ...
    return (<section>
      ...
      {data && (<div className="grid gap-3 border px-3 rounded-xl">
          ...
          <h2>Items</h2>
          <ul>
            {data.response_data.items.map(function (item, index) { return (<li key={index} className="flex justify-between py-2 border-b border-dashed text-sm">
                  <div className="flex flex-col justify-start items-start">
                    {" "}
                    <p><strong>{item.revenue_item || "-"}</strong> </p>{" "}
                    <p className={"".concat(item.status === "Paid" ? "text-green-500 bg-green-50 px-4 border border-green-300 rounded-xl " : "text-yellow-500 bg-yellow-50 px-4 border border-yellow-300 rounded-xl ")}>{item.status || "-"}</p>
                  </div>
