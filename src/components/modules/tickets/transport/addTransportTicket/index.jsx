...
const AddTransportTicketComponent = function () {
  const _a = useState(false), show = _a[0], setShow = _a[1];
  const _b = useState(""), paymentRef = _b[0], setPaymentRef = _b[1];
  ...
  return (<section className="transport_add">
    ...
    <div className="transport-comp">
      ...
      <div className="transport-comp_form">
        <AddTransportTicketForm show={show} setShow={setShow} paymentRef={paymentRef} setPaymentRef={setPaymentRef}
    ...
    {show && (<SuccessModal text="View Receipt" link="/tickets/transport/add/summary"
                            id={"Ref: ".concat(paymentRef, ", Valid for: ").concat(selectedPeriod, ", Payment for: ").concat(selectedProductName)}
                            buttonText="Done"/>)}
  </section>);
};
